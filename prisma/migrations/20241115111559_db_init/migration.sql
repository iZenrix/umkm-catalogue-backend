...
CREATE TABLE `umkm_product` (
    ...
    UNIQUE INDEX `umkm_product_name_key`(`name`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `umkm_product` ADD CONSTRAINT `umkm_product_umkm_id_fkey` FOREIGN KEY (`umkm_id`) REFERENCES `Umkm`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `product_image` ADD CONSTRAINT `product_image_product_id_fkey` FOREIGN KEY (`product_id`) REFERENCES `umkm_product`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Location` ADD CONSTRAINT `Location_umkm_id_fkey` FOREIGN KEY (`umkm_id`) REFERENCES `Umkm`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
